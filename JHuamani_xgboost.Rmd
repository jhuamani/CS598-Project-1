...
clean_data = function(data, forLinear = TRUE){
  ...
  # Hot encode the data
  if (forLinear){
    data_mod = dummyVars(~ .,data=data, fullRank=TRUE)
  }
  else {
    data_mod = dummyVars(~ .,data=data)
  }
...
cv.out = cv.glmnet(train_mat, train_Y, alpha = 1)

# Output features w/ non-zero coefficients
sel.vars = predict(cv.out, type="nonzero",s = cv.out$lambda.1se)[,1]

cv.out <- cv.glmnet(train_mat[, sel.vars], train_Y, alpha = 0)
tmp <-predict(cv.out, s = cv.out$lambda.min, newx = test_mat[,sel.vars])
...
```{r}
# Check correlation b/w all numeric columns and any items w/ a threshold above 0.7
corr_mat = round(cor(trainData),digits=2)
for (name in colnames(corr_mat)){
  corr_check = corr_mat[which(abs(corr_mat[,name]) < 0.3),name]
  if (length(corr_check)>1 && name == "Sale_Price"){
    print(name)
    print(corr_check)
  }
}

# Garage_Area and Garage_Cars are very highly correlated, we may be able to remove one of these columns
```
