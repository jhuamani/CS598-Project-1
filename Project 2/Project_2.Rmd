...
preprocess.svd <- function(train, n.comp){
  train <- train %>%  select(Store, Dept, Date, Weekly_Sales) %>%  spread(Date, Weekly_Sales)
  train[is.na(train)] <- 0
  
  train_svd = NULL

  for(mydept in unique(train$Dept)){
    dept_data <- train %>% filter(Dept == mydept)
  ...
  train_svd <- train_svd %>% gather(Date, Weekly_Sales, -Store, -Dept) %>% mutate(Date = as_date(Date))
...
# read in train / test dataframes from readr
train <- read_csv('train_ini.csv')
test <- read_csv('test.csv')

train_holidays <- train %>%  select(Date, IsHoliday) %>% distinct()

n.comp = 8

train <- preprocess.svd(train, n.comp)
train <- train %>% left_join(train_holidays, by = c("Date"))
...
for (t in 1:num_folds) {
  ...
  new_train_holidays <- new_train %>%  select(Date, IsHoliday) %>% distinct()
  
  new_train <- preprocess.svd(new_train, n.comp)
  new_train <- new_train %>% left_join(new_train_holidays, by = c("Date"))
